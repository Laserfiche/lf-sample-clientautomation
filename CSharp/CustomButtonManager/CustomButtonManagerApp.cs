...
using PageSet = Laserfiche.ClientAutomation.PageSet;
using SortDirection = Laserfiche.ClientAutomation.SortDirection;
...
namespace Laserfiche.Samples
{
    static class CustomButtonManagerApp
    {
        ...
        static bool MainHandler(string[] _args)
        {
            ...
            if (args.Count > 0)
            {
                ...
                {
                    ButtonClick(buttonid, pid, hwnd, command, selectedentries, selectedpages);
                    return true;
                }
        ...
        public static void SetupToolbar(bool silent)
        {
            ...
            string argsbase = " -buttonclick -hwnd \"%(hwnd)\" -pid \"%(PID)\" ";
        ...
        static bool ButtonClick(int buttonid, int pid, int hwnd, string command,
                                ...
                                if (window.GetWindowType() == ClientWindowType.Main)
                                {
                                    ...
                                    if (listEntryIDs.Count == 1)
                                    {
                                        ...
                                        else if (command == "searchbyname")
                                        {
                                            ...
                                            ISession pRASession = Util.GetRASession(repoconn);
                                            EntryInfo pLFCurrentEntry = Entry.GetEntryInfo(listEntryIDs[0], pRASession);
                                    ...
                                    else if (command == "uponelevel")
                                    {
                                        ...
                                        {
                                            ISession pRASession = Util.GetRASession(repoconn);
                                            FolderInfo pLFCurrentFolder = Folder.GetFolderInfo(nCurrentFolderID, pRASession);
                                            int pLFParentFolderId = pLFCurrentFolder.ParentId;
                                            if (pLFParentFolderId > -1)
                                    ...
                                    else if (command == "setcolumns")
                                    {
                                        ...
                                        options.Columns.Add(new ClientColumn((int)SystemColumn.Tags, 100));
                                        options.Columns.Add(new ClientColumn((int)SystemColumn.ModifierName, 100));
                                    ...
                                    else if (command == "setfieldcolumns" && listEntryIDs.Count > 0)
                                    {
                                        ...
                                        SetColumnsOptions options = new SetColumnsOptions();
                                        options.Columns.Add(new ClientColumn((int)SystemColumn.TemplateName, 100));

                                        foreach (int entryID in listEntryIDs)
                                        {
                                            EntryInfo pLFCurrentEntry = Entry.GetEntryInfo(entryID, pRASession);
                                            if (pLFCurrentEntry.EntryType == EntryType.Shortcut)
                                            {
                                                int targetEntryId = ((IShortcutInfo)pLFCurrentEntry).TargetId;
                                                pLFCurrentEntry = Entry.GetEntryInfo(targetEntryId, pRASession);
                                            }

                                            FieldValueCollection pLFFielddata = pLFCurrentEntry.GetFieldValues();
                                            string pLFTemplateName = pLFCurrentEntry.TemplateName;
                                            if (!string.IsNullOrEmpty(pLFTemplateName))
                                            {
                                                var pLFTemplate = Template.GetInfo(pLFTemplateName, pRASession);
                                                if (pLFTemplate != null && pLFTemplate.FieldCount > 0)
                                                {
                                                    var fields = pLFTemplate.Fields;
                                                    foreach(var pLFField in fields)
                                                    {
                                            ...
                                            foreach(var field in pLFFielddata)
                                            {
                                                ClientColumn column = new ClientColumn(field.Key, 100);
                                                options.Columns.Add(column);
                                            }
                                        }
    ...
    static class Util
    {
        public static ISession GetRASession(RepositoryConnection repoconn)
        {
        ...
        static void OpenFindRefresh(string[] args)
        {
            ...
            Server pLFServer = new Server(server);
            RepositoryRegistration pLFDatabase = new RepositoryRegistration(server, repository);
            ...
            using (ClientManager lfclient = new ClientManager())
            {
                ...
                foreach (ClientInstance _client in clients)
                {
                    ...
                    {
                        if (string.Equals(repo.RepositoryName, pLFDatabase.Name, StringComparison.InvariantCultureIgnoreCase) &&
                            string.Equals(repo.ServerName, pLFDatabase.ServerName, StringComparison.InvariantCultureIgnoreCase))
                        {
                            client = _client;
                            break;
                        }
        ...
        static FolderInfo GetRootFolder()
        {
            using (ClientManager lfclient = new ClientManager())
            {
                // Find an existing client instance that is logged in to the repository
                ...
                foreach (ClientInstance _client in clients)
                {
                    ...
                    foreach (RepositoryConnection repo in repos)
                    {
                        // Retrieve the serialized connection string and use it to initialize the RA Session object
                        string strSerializedConnection = repo.GetConnectionString();
                        ISession session = Session.CreateFromSerializedLFConnectionString(strSerializedConnection);
                        FolderInfo folder = Folder.GetFolderInfo(1, session);
                        return folder;
                    }
                }
            }
